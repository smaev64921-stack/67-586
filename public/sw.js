...
/* Push от сервера. Приходит и когда приложение закрыто: служба браузера
   будит этот воркер, показывает уведомление и снова засыпает. Показать
   что-то обязаны — иначе браузер сочтёт push тихим и со временем отзовёт
   разрешение у сайта. */
self.addEventListener('push', (e) => {
  ...
  const opts = {
    ...
    tag: d.tag || 'lc',
    icon: '/icon-192.png',
    /* Значок в строке состояния Android рисует СИЛУЭТОМ: берёт только
       прозрачность, а цвет заливает свой. Цветная иконка превращалась в
       сплошное пятно, поэтому здесь отдельный одноцветный знак. */
    badge: '/badge-96.png',
    lang: 'ru',
    /* Тот же заказ сменил статус ещё раз — уведомление должно заменить
       прежнее и снова подать голос, а не подменить его молча. */
    renotify: true,
    vibrate: [40, 60, 40],
    data: { url: d.url || '/' }
  };
  /* Фото товара — Android разворачивает его крупно, когда шторку тянут
